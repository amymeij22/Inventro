...
import { getLabUsageRequests, updateLabUsageRequestStatus } from "@/lib/api"
import type { LabUsageRequest } from "@/lib/supabase"

export default function AdminLabUsageRequestsPage() {
  const [requests, setRequests] = useState<LabUsageRequest[]>([])
  ...
  const [currentRequest, setCurrentRequest] = useState<LabUsageRequest | null>(null)
  const [newStatus, setNewStatus] = useState<"pending" | "approved" | "rejected" | "completed">("pending")
  const [isSubmitting, setIsSubmitting] = useState(false)

  // Fetch data permintaan penggunaan lab
  const fetchData = async () => {
    try {
      setIsLoading(true)

      // Fetch permintaan penggunaan lab dengan filter
      const requestsData = await getLabUsageRequests(statusFilter !== "all" ? statusFilter : undefined)
  ...
  const openDetailsDialog = (request: LabUsageRequest) => {
    setCurrentRequest(request)
    setIsDetailsDialogOpen(true)
  }

  // Handler untuk membuka dialog ubah status
  const openStatusDialog = (request: LabUsageRequest, status: "pending" | "approved" | "rejected" | "completed") => {
    setCurrentRequest(request)
    setNewStatus(status)
  ...
  const handleUpdateStatus = async () => {
    ...
    try {
      await updateLabUsageRequestStatus(currentRequest.id, newStatus)
    ...
    <div className="flex min-h-screen flex-col">
        ...
        <main className="flex w-full flex-col overflow-hidden pb-16 md:pb-0">
          <div className="flex-1 space-y-4 pt-2 pb-6">
            <div className="flex items-center justify-between">
              <h2 className="text-2xl font-bold tracking-tight">Permintaan Penggunaan Lab</h2>
            ...
            {isLoading ? (
              ...
              <div className="text-center p-8 border rounded-lg">
                <p className="text-muted-foreground">Tidak ada permintaan penggunaan lab yang ditemukan</p>
              </div>
            ) : (
      ...
      <Dialog open={isDetailsDialogOpen} onOpenChange={setIsDetailsDialogOpen}>
        <DialogContent className="max-w-2xl">
          <DialogHeader>
            <DialogTitle>Detail Permintaan Penggunaan Lab</DialogTitle>
          </DialogHeader>
          {currentRequest && (
            <div className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <h3 className="font-medium">Informasi Pemohon</h3>
                ...
                <div>
                  <h3 className="font-medium">Informasi Penggunaan</h3>
                  <div className="mt-2 space-y-1">
                    <p className="text-sm">
                    ...
                    <p className="text-sm">
                      <span className="text-muted-foreground">Waktu:</span> {currentRequest.start_time} -{" "}
                      {currentRequest.end_time}
                    </p>
                    <p className="text-sm">
      ...
      <Dialog open={isStatusDialogOpen} onOpenChange={setIsStatusDialogOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              {newStatus === "approved"
                ? "Apakah Anda yakin ingin menyetujui permintaan penggunaan lab ini?"
                : newStatus === "rejected"
                  ? "Apakah Anda yakin ingin menolak permintaan penggunaan lab ini?"
                  : "Apakah Anda yakin ingin menandai permintaan penggunaan lab ini sebagai selesai?"}
            </DialogDescription>
          </DialogHeader>
          {currentRequest && (
            <div className="py-4">
              <div className="border rounded-md p-4">
                ...
                <p className="text-sm text-muted-foreground">
                  Waktu: {currentRequest.start_time} - {currentRequest.end_time}
                </p>
              </div>
            </div>
          )}
